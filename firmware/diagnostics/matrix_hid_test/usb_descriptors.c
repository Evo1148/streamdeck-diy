...
enum { USB_VID = 0xCAFE, USB_PID = 0x4005, USB_BCD = 0x0200 };
...
static char const* const strings[] = {
    NULL, "StreamDeck DIY", "StreamDeck Matrix Test", NULL,
};
...
uint16_t const* tud_descriptor_string_cb(uint8_t index, uint16_t language_id) {
    ...
    utf16_descriptor[0] = (uint16_t)((TUSB_DESC_STRING << 8) | (2 * count + 2));
